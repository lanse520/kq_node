...
const moment = require('moment'); // NOTE: import moment module
...
function parseKQ(html) {
    ...
    while (true) {
        ...
        let m = rex.exec(html);
        if (m) {
            if(!member_clocks[m[3]]){
                member_clocks[m[3]]=new Array();
            }
            if(!member_clocks[m[3]][moment(m[4]).format("M/D/Y")]){
                member_clocks[m[3]][moment(m[4]).format("M/D/Y")]=new Array();
            }
            member_clocks[m[3]][moment(m[4]).format("M/D/Y")].push(m[4]);
...
function AllStatus(beginDate, endDate){
    let date_arr = Array();//工作日
    let start = moment(beginDate);
    let end = moment(endDate);
    let current = start;
    while (start <= end) {
      const day = current.day();
      if (day > 0 && day < 6) {
        date_arr[current.format("M/D/Y")] = '工作日';
      }
      current = current.add(1, 'd');
    }
    console.log('All Status:');
    console.log(`/Name /Date /Clock In Time /Clock Out Time /Status`);
    for(let name in member_clocks){
        for(let k in date_arr){
            let status = "";
            let data_length = "";
            if(member_clocks[name][k]){
                data_length = member_clocks[name][k].length;
                if(data_length == 1 || moment(member_clocks[name][k][0]).isSame(member_clocks[name][k][data_length-1])){
                    status = "只刷一次";
                }else if(moment(member_clocks[name][k][data_length-1]).isAfter(k+' 8:50:59')){
                    status = "迟到";
                }else if(moment(member_clocks[name][k][0]).isBefore(k+' 16:50:00')){
                    status = "早退";
                }else if(moment(member_clocks[name][k][0]).diff(member_clocks[name][k][data_length-1],'hour',true)<9){
                    status = "工时不足";
                }else{
                    status = "正常";
                }
            ...
            console.log(`${name} ${k} ${member_clocks[name][k]?moment(member_clocks[name][k][data_length-1]).format('HH:mm:ss'):'*'} ${member_clocks[name][k]?moment(member_clocks[name][k][0]).format('HH:mm:ss'):'*'} ${status}`);
        };
    };
}

function askAll() {
    inquire('2021-1-1', '2021-1-20', 'Lance Li', false,
        ()=> inquire('2021-1-1', '2021-1-20', 'ANNE', false,
        ()=> inquire('2021-1-1', '2021-1-20', 'Xuqian Hu', false,
        ()=> inquire('2021-1-1', '2021-1-20', 'Carlos Jiang', false,
        ()=> inquire('2021-1-1', '2021-1-20', 'Jack QP Zhang', false,
        ()=> inquire('2021-1-1', '2021-1-20', 'Joy Yang', false,
        function() { AllStatus('2021-1-1', '2021-1-20');console.log("All done.") } ))))));
}
